...
def main(argv):  
  ...
  try:
    ...
    output.hosts(hosts)
    output.IEComponents(IEComponents)
    output.IEToolbars(IEToolbars)
    output.registers(regs)
    output.LSP(num_entries, LSPs)
    output.DNS(primaryDNS, secondaryDNS)
    ...
    output.startups(global_startups, user_startups)
    output.winlogon(winlogon_entries)
    output.finishLog()
  except Exception as err:
